...
function TournamentTab({ summoners }: { summoners: SummonerMap }) {
  ...
  const fetchTeamsAndMatches = useCallback(async (tid: number) => {
    const [{ data: t }, { data: m }] = await Promise.all([
      supabase.from('tournament_teams').select('*').eq('tournament_id', tid).order('id', { ascending: true }),
      supabase.from('tournament_matches').select('*').eq('tournament_id', tid).order('created_at', { ascending: true }),
    ])
    if (t) setTeams(t)
    if (m) setMatches(m)
  }, [])
  ...
  useEffect(() => {
    if (selectedId) fetchTeamsAndMatches(selectedId)
    else { setTeams([]); setMatches([]) }
  }, [selectedId, fetchTeamsAndMatches])

  const createTournament = async () => {
    const n = newName.trim()
    if (!n) { setError('대회 이름을 입력해주세요.'); return }
    if (!checkPassword()) return
    setError('')
    const { data } = await supabase.from('tournaments').insert([{ name: n, status: 'ongoing' }]).select()
    setNewName('')
  ...
  const deleteTournament = async (id: number) => {
    if (!checkPassword()) return
    if (!confirm('이 대회와 관련된 팀/경기 기록이 모두 삭제돼요. 계속할까요?')) return
  ...
  const handleMemberSearch = (v: string) => {
    setMemberSearch(v)
    if (v.trim()) {
      const matched = Object.keys(summoners).filter(n => n.includes(v.trim()) && !selectedMembers.includes(n))
      setMemberSuggestions(matched.slice(0, 5))
    } else setMemberSuggestions([])
  }

  const addMember = (n: string) => {
    setSelectedMembers(prev => prev.includes(n) ? prev : [...prev, n])
    setMemberSearch('')
    setMemberSuggestions([])
  }

  const removeMember = (n: string) => setSelectedMembers(prev => prev.filter(m => m !== n))

  const addTeam = async () => {
    if (!selectedId) return
    const n = teamName.trim()
    if (!n) { setError('팀명을 입력해주세요.'); return }
    if (selectedMembers.length === 0) { setError('팀원을 최소 1명 이상 선택해주세요.'); return }
    if (!checkPassword()) return
    setError('')
    await supabase.from('tournament_teams').insert([{ tournament_id: selectedId, name: n, members: selectedMembers, wins: 0, losses: 0, eliminated: false }])
    setTeamName('')
    setSelectedMembers([])
    await fetchTeamsAndMatches(selectedId)
  }
  ...
  const recordMatch = async () => {
    if (!selectedId) return
    if (!roundLabel.trim()) { setError('라운드명을 입력해주세요.'); return }
    if (teamAId === '' || teamBId === '' || teamAId === teamBId) { setError('서로 다른 두 팀을 선택해주세요.'); return }
    if (winnerId === '') { setError('승리 팀을 선택해주세요.'); return }
    setError('')
    ...
    await supabase.from('tournament_matches').insert([{
      ...
      team_b_id: teamBId,
      winner_id: winnerId,
      score_a: parseScoreInput(scoreA),
      score_b: parseScoreInput(scoreB),
    }])

    const loserId = winnerId === teamAId ? teamBId : teamAId
    const winnerTeam = teams.find(t => t.id === winnerId)
    const loserTeam = teams.find(t => t.id === loserId)
    if (winnerTeam) await supabase.from('tournament_teams').update({ wins: winnerTeam.wins + 1 }).eq('id', winnerTeam.id)
    if (loserTeam) {
      // 더블 엘리미네이션: 2패 하면 탈락
      const newLosses = loserTeam.losses + 1
      await supabase.from('tournament_teams').update({ losses: newLosses, eliminated: newLosses >= 2 }).eq('id', loserTeam.id)
    }
  ...
  const deleteMatch = async (m: TournamentMatch) => {
    if (!checkPassword()) return
    if (!selectedId) return
    if (!confirm('이 경기 기록을 삭제할까요? 팀 승/패 기록도 되돌려져요.')) return
    const winnerTeam = teams.find(t => t.id === m.winner_id)
    const loserId = m.winner_id === m.team_a_id ? m.team_b_id : m.team_a_id
    const loserTeam = teams.find(t => t.id === loserId)
    if (winnerTeam) await supabase.from('tournament_teams').update({ wins: Math.max(0, winnerTeam.wins - 1) }).eq('id', winnerTeam.id)
    if (loserTeam) {
      const newLosses = Math.max(0, loserTeam.losses - 1)
      await supabase.from('tournament_teams').update({ losses: newLosses, eliminated: newLosses >= 2 }).eq('id', loserTeam.id)
    }
    await supabase.from('tournament_matches').delete().eq('id', m.id)
    await fetchTeamsAndMatches(selectedId)
  }

  const setChampion = async (name: string) => {
    if (!selectedId) return
    if (!checkPassword()) return
    await supabase.from('tournaments').update({ status: 'completed', champion: name }).eq('id', selectedId)
    await fetchTournaments()
  }
  ...
  const reopenTournament = async () => {
    if (!selectedId) return
    if (!checkPassword()) return
    await supabase.from('tournaments').update({ status: 'ongoing', champion: null }).eq('id', selectedId)
    await fetchTournaments()
  }

  const teamNameOf = (id: number | null) => teams.find(t => t.id === id)?.name ?? '?'
  ...
  return (
    <div>
      <div className="card">
        <div className="card-title">🏆 대회 관리</div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <input
            value={newName}
            onChange={e => setNewName(e.target.value)}
            placeholder="새 대회 이름 (예: 2026 서머 내전 MSI)"
            onKeyDown={e => e.key === 'Enter' && createTournament()}
            style={{ flex: 1 }}
          />
          <button className="btn btn-gold" onClick={createTournament}>대회 생성</button>
        </div>
        {tournaments.length > 0 && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {tournaments.map(t => (
              <button
                key={t.id}
                className={`btn btn-sm${selectedId === t.id ? ' btn-gold' : ''}`}
                onClick={() => setSelectedId(t.id)}
              >
                {t.name}{t.status === 'completed' ? ` 🏆${t.champion ?? ''}` : ''}
              </button>
            ))}
          </div>
        )}
      ...
      {!selected ? (
        <div className="empty">대회를 생성하거나 선택해주세요</div>
      ) : (
        <>
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div className="card-title" style={{ marginBottom: 0 }}>
                {selected.name}{selected.status === 'completed' && ` — 🏆 우승: ${selected.champion}`}
              </div>
              <button className="btn btn-danger btn-sm" onClick={() => deleteTournament(selected.id)}>대회 삭제</button>
            </div>
            {selected.status === 'completed' && (
              <button className="btn btn-sm" style={{ marginTop: 8 }} onClick={reopenTournament}>대회 다시 열기 (우승 취소)</button>
            )}
          </div>

          <div className="card">
            <div className="card-title">팀 등록 (등록된 소환사 중에서 선택)</div>
            <input
              value={teamName}
              ...
              {memberSuggestions.length > 0 && (
                <div style={{
                  ...
                  {memberSuggestions.map(s => (
                    <div key={s} onClick={() => addMember(s)} style={{ padding: '8px 12px', cursor: 'pointer' }}>{s}</div>
                  ))}
                </div>
              )}
            </div>
            {selectedMembers.length > 0 && (
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                {selectedMembers.map(m => (
                  <span key={m} className="badge b-line" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    {m} <span style={{ cursor: 'pointer' }} onClick={() => removeMember(m)}>✕</span>
                  </span>
                ))}
              </div>
            )}
            <button className="btn btn-gold" onClick={addTeam}>팀 추가</button>

            <div style={{ marginTop: 14 }}>
              {teams.length === 0 ? <div className="empty">등록된 팀이 없어요</div> : teams.map(t => (
                <div key={t.id} style={{ marginBottom: 10, padding: '10px 12px', background: 'var(--bg3)', borderRadius: 'var(--radius)', border: '0.5px solid var(--border)', opacity: t.eliminated ? 0.55 : 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, flexWrap: 'wrap', gap: 6 }}>
                    <span style={{ fontWeight: 700 }}>{t.name}{t.eliminated ? ' (탈락)' : ''}</span>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span style={{ fontSize: 12, color: 'var(--text2)' }}>{t.wins}승 {t.losses}패</span>
                      {selected.status === 'ongoing' && <button className="btn btn-sm" onClick={() => setChampion(t.name)}>우승 확정</button>}
                      <button className="btn btn-danger btn-sm" onClick={() => deleteTeam(t.id)}>삭제</button>
                    </div>
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text3)' }}>{t.members.join(', ')}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
              {ROUND_PRESETS.map(r => (
                <button key={r} className={`btn btn-sm${roundLabel === r ? ' btn-gold' : ''}`} onClick={() => setRoundLabel(r)}>{r}</button>
              ))}
            </div>
            <input value={roundLabel} onChange={e => setRoundLabel(e.target.value)} placeholder="라운드명 (직접 입력도 가능)" style={{ marginBottom: 8 }} />
            <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <select value={teamAId} onChange={e => { setTeamAId(e.target.value ? Number(e.target.value) : ''); setWinnerId('') }} style={{ flex: 1, minWidth: 120 }}>
            ...
            {teamAId !== '' && teamBId !== '' && teamAId !== teamBId && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                <span style={{ fontSize: 12, color: 'var(--text2)' }}>승리 팀:</span>
                <button className={`btn btn-sm${winnerId === teamAId ? ' btn-gold' : ''}`} onClick={() => setWinnerId(teamAId)}>{teamNameOf(teamAId as number)}</button>
                <button className={`btn btn-sm${winnerId === teamBId ? ' btn-gold' : ''}`} onClick={() => setWinnerId(teamBId)}>{teamNameOf(teamBId as number)}</button>
              </div>
            )}
            <button className="btn btn-gold" onClick={recordMatch}>경기 기록</button>
          </div>
...
export default function Home() {
  const [tab, setTab] = useState<'team' | 'record' | 'ranking' | 'hall' | 'stats' | 'tournament' | 'summoners'>('team')
  ...
  return (
    <div className="layout">
      ...
      <div className="tabs" style={{ background: 'rgba(6,17,31,0.75)' }}>
        {(['team', 'record', 'ranking', 'hall', 'stats', 'tournament', 'summoners'] as const).map((t, i) => (
          <button key={t} className={`tab${tab === t ? ' active' : ''}`} onClick={() => setTab(t)}>
            {['팀 뽑기', '전적 기록', '전체 랭킹', '명예의 전당', '개인 통계', '대회', '소환사 관리'][i]}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {tab === 'tournament' && <TournamentTab summoners={summoners} />}
